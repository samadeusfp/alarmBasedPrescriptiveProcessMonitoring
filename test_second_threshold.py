...
method = "2_threshold_backward_trained"
    ...
    for c_miss_weight, c_action_weight in cost_weights:
        for c_com_weight in c_com_weights:
            for early_type in ["const", "linear","nonmonotonic"]:
                ...
                conf_thresholds.append(conf['conf_threshold2'])
                conf_thresholds.append(conf['conf_threshold'])
                ...
                for nr_events in range(1, dt_preds.prefix_nr.max() + 1):
                    if nr_events == 1:
                        conf_threshold = conf_thresholds[0]
                    else:
                        conf_threshold = conf_thresholds[1]
